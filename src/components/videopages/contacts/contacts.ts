...
import { ChatPage1, AccountPage, ChatsPage } from '../';
...
export class ContactsPage {
	...
	constructor(private chatService: ChatService, private modalCtrl: ModalController, private navCtrl: NavController) {
	...
	chat(id) {
		this.chatService.getChatByContact(id).then((chat:any) => {
			...
			this.navCtrl.push(ChatPage1, {chatId: chat.id}, {animate: true, direction: 'forward'});
		});
	}
